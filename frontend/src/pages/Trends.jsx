...
import LiveTrendsPanel from "../components/trends/LiveTrendsPanel";
...
const Trends = () => {
  ...
  // Selected trend for Detail Modal & selected idea for AI Script Studio modal
  const [selectedTrend, setSelectedTrend] = useState(null);
  const [selectedIdeaForStudio, setSelectedIdeaForStudio] = useState(null);
  ...
  return (
    <div className="space-y-6 pb-8">
      ...
      <div className="flex items-center gap-2 p-1.5 bg-surface-100 dark:bg-dark-surface rounded-2xl border border-surface-200 dark:border-dark-border w-fit">
        ...
        >
          <IoRadioOutline className={`w-4 h-4 ${activeTab === "live" ? "animate-pulse" : ""}`} />
          <span>Live YouTube & Google Trends</span>
        ...
        >
          ...
          <span>Curated Market Trends</span>
          <span className="text-xs px-2 py-0.5 rounded-full bg-surface-200 dark:bg-dark-surface-light text-text-secondary dark:text-dark-text-muted">
            {trends.length}
          </span>
        </button>
      </div>
      ...
      {activeTab === "live" ? (
        <LiveTrendsPanel onSelectTrend={handleExplore} />
      ) : (
        <>
          {/* Stats Cards */}
      ...
      {selectedTrend && (
        <TrendDetailModal
          ...
          onDevelopScript={(idea) => navigate("/studio", { state: { idea } })}
        />
      )}

      {/* AI Script Studio Modal */}
      {selectedIdeaForStudio && (
        <ScriptStudioModal
          idea={selectedIdeaForStudio}
          onClose={() => setSelectedIdeaForStudio(null)}
        />
      )}
    </div>
  );
